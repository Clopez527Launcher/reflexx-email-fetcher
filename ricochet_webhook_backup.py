...
def ricochet_webhook():
    ...
    try:
        ...
        inbound_add  = dur if call_type == CALL_TYPE_INBOUND  else 0
        outbound_add = dur if call_type in (1, 2, 3, 4, 5, 6) else 0

        # prefer truth from our DB (guards against misconfigured payloads)
        ...
        if inbound_add or outbound_add:
            ...
            print("[ricochet] unknown/ignored call_type:", call_type, "dur:", dur)

    except Exception as e:
        # we already saved raw_json; keep ack 200 so Ricochet doesn't retry forever
