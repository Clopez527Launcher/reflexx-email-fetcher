...
def ce_buckets():
    ...
    try:
        # 1) Find the latest date with any CE z-score for this manager
        cur.execute("""
            ...
            WHERE u.manager_id = %s
              AND (
                    fd.phone_ce_l7_z IS NOT NULL
                 OR fd.quote_ce_l7_z IS NOT NULL
                 OR fd.movement_ce_l7_z IS NOT NULL
              )
        """, (mgr_id_int,))
        ...
        cur.execute("""
            SELECT
                ...
                fd.phone_ce_l7_z,
                fd.quote_ce_l7_z,
                fd.movement_ce_l7_z AS move_ce_l7_z
            FROM fact_daily fd
            JOIN users u ON u.id = fd.user_id
            WHERE u.manager_id = %s
              AND fd.date = %s
...
def login():
    ...
    # Redirect as before
    return redirect(url_for("manager_portal" if user["role"] == "manager" else "dashboard"))
